...
void TriMesh::readObj(const std::string& filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		if (type == "vn") {
			...
			//用法向导出颜色分量
			// vertex_colors:存储顶点颜色数据,用法向量的数值赋值
			vertex_colors.push_back(glm::vec3(a, b, c));
		}

		// “f”开头表示面的数据，记录的是顶点索引，每个面由多个顶点组成
		//这里每个顶点的3个数值分别表示顶点索引下标、纹理的UV索引下标、法向量索引下标。
		//索引号分别用左斜线(/)隔开，每个顶点的数据用空格分开，即格式：f 顶点索引/uv点索引/法线索引
		if (type == "f") {
			float a, b, c;
			...
			// color_index:存储颜色下标索引,用法向量的数值赋值
