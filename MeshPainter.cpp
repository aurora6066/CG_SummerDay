...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera){
	...
	camera->projMatrix = camera->getProjectionMatrix(false); // 让相机能拉近拉远
    ...
    if(mesh->getTranslation().y>0 && mesh->getTranslation().y < 9.5 && mesh->getScale().x<50 ) //&& mesh->getTranslation().y < light->getTranslation().y && mesh->getTranslation().z>-15 && mesh->getTranslation().z < 15 && mesh->getTranslation().x > -15 && mesh->getTranslation().x < 15)
    {   
         // 地面上，光源下的物体才有阴影
        //绘制阴影
